...
\section{前言}
	...
	字符串匹配的相关算法旨在解决在一个较大规模的文本中查询模式串匹配情况的相关问题。
	这里的模式串可能是一系列单词，也可能是一个强大的正则表达式。解决这类问题存在很多算法，
	而这些算法往往包括\Emph{预处理}以及\Emph{搜索}两个阶段。
	其中一部分算法在搜索阶段往往采用\Emph{自动机}这一载体，自动机的优势
	主要在于通过状态的跃迁加快查找的速度，同时支持在线查找，可以处理大文件。
	
	本文涉及的算法主要使用\Emph{最小化的确定性有穷自动机}，用来处理\Emph{re-matching}、
    \Emph{dictionary-matching}以及\Emph{index-matching}等问题。

\section{符号说明}
	
	简单对使用的符号进行说明，后面的自动机算法都尽量采用形式化的方式描述。
	
\subsection{Alphabet and words}
	
	% \noindent
    $A$表示一个字符集合，$a$表示$A$中的一个字符元素。				\\
	$u$表示一个单词，$|u|$表示长度，$u_j$和$u[j]$均表示$u$中的第$j$个字符。	\\
	$u \cdot v$或$uv$表示单词$u$连接单词$v$，$u^0 = \varepsilon$。\\
	当$u,v,w$满足$u = wv$时，有$uv^{-1} = w, w^{-1}u = v$	\\
	当$\exists u_0, u_1, u=u_0vu_1$成立时，称$v$是$u$的\Emph{factor}。
	此时若$v \neq u$并且$u_0=\varepsilon \text{ or } u_1=\varepsilon$，
	我们称$v$是$u$的\Emph{proper factor}。
...
\subsection{Languages}
	...
	举个例子，$W = Pref(abcbc)$，那么字符串$abc$的右端上下文就是$\{ \varepsilon, b, bc \}$
...
\subsection{Finite automata}
	...
	\[
		...
		\left\{
		\begin{aligned}
			&p,		  &\text{ if } u=\varepsilon,	\\
			&\delta(\delta(p, a), v), &\text{ if } u=av,	\\
			&undefined,  &&\text{ if }.
		\end{aligned}
		\right .
	\]
	其实这个递推式描述了在自动机FA上搜索$u$的过程。
...
\section{DFA的抽象数据结构}
	...
	我们将使用不同方法查询$\delta(p,a)$的时间花费称为$delay$, $delay$表示在查询下一个字符$a$产生的状态转移所花费的时间。
	一般来说，允许使用索引表示状态转移的数据结构的$delay$为常数，如transition matrix；而基于比较的数据结构的查询时间为
	$O(\log |A|)$。
	
\subsection{Transition matrix}

	转移矩阵式最简单的DFA的实现方式，这也不难想象。
	还是将DFA想象成一个DAG，这其实就是个邻接矩阵。
...
\subsection{Adjancy lists}
	...
	其实这里一般写成$O(\log |A|)$就好了。
	当$|Q| > |A|$时，每个结点的出度至多为$|A|$。
	而当$|Q| < |A|$时，每个结点的出度至多为$|Q|$。
...
\label{sub:failure_function}
	...
	\Emph{Failure function}最为重要，十分高效。后面的几种自动机模型都一定程度上使用了failure function。
	...
	更为重要的是，使用均摊进行分析可以发现时间复杂度不会因此而提高。
	...
	\begin{defn}
		...
		\[
			...
			\left\{
			\begin{aligned}
				&\gamma(p, a), 		&\text{ if } \gamma(p, a) \neq \const{nil},	              \\
				&\delta(f(p), a), 	&\text{ if } \gamma(p, a) = \const{nil} \text{ and } f(p) \neq \const{nil},	\\
				&\id{i},			&\text{ otherwise }.
			\end{aligned}
			\right .
		\]
	...
	这里$E' = \{(p,a,q) | (p,a,q) \in E \text{ and } \gamma(p,a) \text{ is defined } \}$.
...
\subsection{Table-compression}
	...
	\[
		...
		\left\{
		\begin{aligned}
			&target[base[p]+a], 	&\text{ if } check[base[p]+a] = p		\\
			&\delta(fail(p), a),	&\text{ if } check[base[p]+a] \neq p
		\end{aligned}
		\right .
	\]
...
\subsection{RE自动机}
	...
	\begin{thm} 令$x$表示一个正则表达式，一定存在一个可以识别$x$并满足如下条件的$\varepsilon$-NFA：
		\begin{enumerate}[(1)]
			...
			\item 每个状态的入度和出度的总和不超过$2$，并当且仅当标号为$\varepsilon$时总和为$2$。
			
		\end{enumerate}
	\end{thm}
	
	这个定理还是比较重要的，为RE自动机算法的空间复杂度$O(|x|)$和时间复杂度$O(|x||y|)$奠定了基调。
...
\subsection{使用RE自动机搜索}
	...
	这里面有个稍微难理解的东西$\varepsilon$-闭包。因为$\varepsilon$-NFA中的状态转移可能由$\varepsilon$引发，
	因此，对于$\delta(p,\varepsilon) = q$，当搜素到$p$时，同时也要把$q$加入带搜索集合中。
	因为，这个状态有发生的可能。（DFA可以解决这个问题，但是会带来新的问题——即状态爆炸。）
	...
	这样一个自动机的基本功能是用来检测字符串$y$是否可以被正则表达式$x$所表示，这样一个过程称为\proc{Tester}。
	\begin{codebox}
	...
	\begin{proof}
		其实只能说\proc{Transitions}和\proc{Closure}可以在$O(|Q|)=O(|x|)$中实现，这样\proc{Tester}的复杂度就是$O(|y||x|)$。
	\end{proof}
	
	\proc{Tester}只能做判定$y$是否匹配$x$，故还得回到最初的问题，识别所有可以匹配$x$的$fact(y)$。
	将这样的$fact(y)$在y中的右边界的位置称为$end-point$。
	做法很简单，在每次\proc{Tester}的循环中都将初始状态$\id{i}$加入$C$中，表示每次搜索可能都是从初始状态开始。
	这样就得到了过程\proc{Matcher}。
    ...
    \Procname{$\proc{Matcher}(x,y)$}
        ...
        \li	\For $j \gets 1$ \To $\attrib{y}{length}$
        \li     \Do
                    $C \gets \proc{Closure}(E, \proc{Transition}(E, C, y[j]) + \{ \id{i} \})$
        \li         \func{occurrence} \If t \kw{in} C
                \End
    \end{codebox}
...
\subsection{时间和空间的trade-off}
	...
	上述方法使用了$\varepsilon$-NFA这种自动机模型，其实也可以使用$DFA$。
	但是在对状态进行确定化时，往往会产生空间爆炸。
	如$x = a(a+b)^{m-1}$，识别$A^*Long(x)$的$DFA$需要$2^m$个状态。
	因为它需要记录$m$个后缀字符。因此，在构建$DFA$时往往采用延迟构建这种技巧。
...
\label{subsub:sam_online_construct}
	...
	\Procname{$\proc{SA-Extend}(\ell)$}
		...
		\li	\Else
				$q' \gets \proc{State-Creation()}$
		\li		\For \id{b} such that $\delta(q, b) \neq \const{nil}$
		\li		\Do 
...
\label{subsub:strmacth_end_factor}
	...
	\proc{Ending-Factors}过程计算在$y$的每个位置出现的$x$的最长子串，更确切地说是这个子串的长度。使用$\ell_k$表示
	\[
		\forall k \in [0, |y|], \ell_k = \max \{ |w| | w \in \Suff(y_1 y_2 \cdots y_k) \cap Fact(x) \}
	\]
	\proc{Ending-Factors}可以在线计算最长结尾子串的序列$(\ell_k)_{0 \le k \le |y|}$。
	它以由$\{0, \cdots, |x|\}$的字符组成的字符串作为输出结果。
	~\ref{sub:sam_property}引入的函数$length_x$用来对使用$suffix\ link$遍历后对当前长度进行恢复。
	
	\proc{Ending-Factors}函数的核心是使用$F$数组在转换过程中进行计算。
	\begin{codebox}
	\Procname{$\proc{Ending-Factors}((Q,i,T,E), Length, F, y)$}
		\zi \Comment let $\delta$ be the transition function of $(Q, i, T, E)$
		\li	$(\ell, p) \gets (0, i)$
		\li	$L \gets 0$
		\li	\For $j \gets 1$ \To \attrib{y}{length}
	...
	\begin{thm}
		\proc{Ending-Factors}过程计算字符串$y$中的$x$的最长结尾子串的时间复杂度为$O(|y| \times \log |A|)$，空间复杂度为$O(|x|)$。
