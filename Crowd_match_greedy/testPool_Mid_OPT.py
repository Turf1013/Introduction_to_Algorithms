...
def testPool(distId):
	...
	orderN, caseN = 1, 5
	numRateList = [0.25, 0.5, 1.0, 2.0, 4.0]
	numCountList = [100, 200, 400, 500]
	degRateList = [0.01, 0.02, 0.05, 0.10, 0.20, 0.5, 0.75, 0.90, 1.00]
	umaxList = [100, 1000]

	#farFilePrefix = "/home/turf/tmp/data" + str(distId)
	farFilePrefix = "/home/server/zyx/MedianData/data" + str(distId)
	...
	for execName in execNameList:
		...
		if execName.endswith("Mem"):
			...
			logFilePrefix = "/home/server/zyx/MedianLog/%s_mem_%d" % (name, distId)
		else:
			name = execName
			logFilePrefix = "/home/server/zyx/MedianLog/%s_clk_%d" % (name, distId)
