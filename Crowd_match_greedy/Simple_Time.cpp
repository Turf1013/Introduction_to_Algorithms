...
struct Hungarian_t {
	...
	void build(const vector<node_t>& tasks, const vector<node_t>& workers) {
		...
		for (int i=0; i<Wsz; ++i) {
			for (int j=0; j<Tsz; ++j) {
				double cost = getCost(i, j, tasks, workers);
				if (dcmp(cost) > 0) {
					E.push_back(edge_t(i, j, cost));
				}
			}
		}
		sort(E.begin(), E.end());
	...
	while (seqN--) {
		...
		if (seqN==0 || node.begTime>=curBound) {
			...
			if (Tsz >= Wsz) {// x is worker
				...
				// remove node which is beyond deadline
				markDeadNode(visitX, tasks, curBound);
				markDeadNode(visitY, workers, curBound);
...
int main(int argc, char* argv[]) {
	...
	#ifdef WATCH_MEM
	printf("PureTime %.6lf %.6lf %.4lf\n", utility, usedTime, usedMemory/1024.0);
	#else
	printf("PureTime %.6lf %.6lf\n", utility, usedTime);
	#endif
	fflush(stdout);
